...
- Agent1 查询图数据库辅助澄清。
- Agent1 必须以 CrewAI Agent 形式运行，能够主动调用 `nebula_graph_query` 和 `problem_reporter`。
...
CrewAI scheduler_agent
  ↓ 调用 tools
nebula_graph_query / problem_reporter
  ↓ 输出草稿 JSON
...
```json
{
  "task_contract": {
    ...
    "required_capabilities": [
      {
        "name": "nebula_graph_query",
        "required": true,
        "owner": "Agent2",
        "purpose": "Agent2 使用与 Agent1 相同的 nebula_graph_query 工具，自主查询图数据库并确认实体、关系、字段位置和图谱缺口。",
        "acceptance_criteria": [
          "必须记录查询到的实体、关系和缺口。",
...
- `agent2_planning_policy.must_use_same_graph_tool = nebula_graph_query`，表示 Agent1/Agent2 使用同一个图数据库查询工具，Agent2 需要自己调用该工具复核图谱范围。
...
|---|---|---|
| `nebula_graph_query` | 是 | Agent2 用同一个图谱工具确认实体、关系、字段位置和图谱缺口 |
...
|---|---|---|---|
| `nebula_graph_query` | `tools/nebula_graph_query.py` | 是 | 根据业务词查询图数据库，返回 schema、vertices、edges，并由 Agent1 基于结果生成澄清内容 |
...
| 工具名 | 当前位置 | 状态 | 说明 |
|---|---|---|---|
| `nebula_graph_query` | `tools/nebula_graph_query.py` | 已统一 | 按合同查询图谱实体和关系；Agent1 使用 `output_format=json` 获取结构化图谱，Agent2 默认获取文本摘要 |
...
- Agent1 默认不要求用户选择 space；`nebula_graph_query` 会先调用 `GET /spaces`，再按用户问题和各 space 的 tag / edge 命中度自动选择。
...
`nebula_graph_query` 直接返回 API 数据，不做澄清推理。
...
- 改造 `tools/nebula_graph_query.py`：
  - 工具名改成 `nebula_graph_query`。
...
- CrewAI Agent1 主动调用 `nebula_graph_query`。
...
- Agent1 CrewAI Task 能调用 `nebula_graph_query`。
- 模糊问题触发图数据库查询。
...
- `agents/agent1.py`：`run_agent1_clarification` 本地入口，会先判断是否需要查询图谱；业务问题会调用 `nebula_graph_query` 获取图谱数据，再交给确定性核心生成澄清结果。
- `agents/agent1.py`：基于 `nebula_graph_query` 返回图谱数据提取 `转化` 关系，并生成图谱来源的澄清选项。
...
- `agents/agent1.py`：Agent1 在 `task_contract.graph_query_boundary`、`graph_entity_hints`、`graph_relationship_hints` 中提供图谱边界和提示；Agent2 必须使用同一个 `nebula_graph_query` 工具自己查询图数据库。
...
- `tools/nebula_graph_query.py`：Agent1 和 Agent2 共用的 `nebula_graph_query` 工具，优先支持正式 Graph API + `Authorization: <API Key>` + Apipost 风格 `User-Agent`，兼容 `raw` Nebula 错误文本；默认可通过 `GET /spaces` 自动选择 graph space；仍保留 `MEDGRAPH_JSON_PATH` 本地 JSON fallback、NebulaGraph 查询和同结构模拟降级；设置 `GRAPH_API_STRICT=1` 时禁用本地 JSON/mock fallback，真实 API 失败会返回结构化错误。Agent1 调用时传 `output_format=json`，Agent2 默认使用文本摘要。
